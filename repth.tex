...
\begin{enumerate}[(a)]
    ...
    \item the dual $V^* = \Hom(V,\CC)$. This is a little tricky though. The action of $G$ on $V^*$ must be such that it preserves the natural inner product, denoted by $\<\ ,\ \>$, between them. So, if we have a representation $(V^*,\rho^*)$ and $(V,\rho)$, then we should have
        ...
        \begin{insight}
            Remember the definition of the tranpose map. If we have a map $f: V\to W$, then the tranpose of $\rho$ is a map $^t\rho: W^*\to V^*$ such that
            \begin{align}
...
\begin{insight}
    ...
    \begin{align}
        x^3=1,\quad y^2=1,\quad (yx)^2=1.
    \end{align}
    You can tell that $y$ is a 2-cycle and $x$ is a 3-cycle.
...
\end{enumerate}
We now want to characterize any arbitrary representation of $S_3$. 
